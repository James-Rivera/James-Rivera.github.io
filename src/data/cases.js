const detailedCases = {
  flowpress: {
    ...
    sections: [
      ...
      { title: 'How the architecture changed', lead: 'The first version put the upload page on the public internet and the backend in my homelab. Connectivity and adoption problems pushed the file path back inside the shop.', items: [['V1 — deployed','Public upload page → Cloudflare tunnel → homelab Docker backend → files synced to the shop PC.'],['V2 — in progress','MikroTik hotspot → on-site upload server → local job state → staff releases the print. The public route remains as fallback.']] },
      ...
      { title: 'Where it stands', items: [['Working now','The upload page, job queue and print path are used at the shop.'],['Tested, not adopted','The captive-portal upload path and the custom admin queue staff chose not to use.'],['Still planned','A shop-local default path, clearer job states and monitoring.'],['What I would do differently','Watch staff work for a week before designing the queue.']] }
    ]
  },
  konektado: {
    eyebrow: 'Case study',
